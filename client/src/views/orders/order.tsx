...
const Orders = () => {
  ...
  const validTypes = ["sold", "purchase"];
  ...
  return (
    <div className="w-full md:max-w-6xl mx-auto px-3 md:px-0">
      <TopNavbar title={`${type==="sold" ? "Sold orders" : "Purchase orders"}`}/>
      <div className="hidden md:block mt-8 mb-4 ">
        <span className="font-medium text-2xl">{`${type==="sold" ? "Sold orders" : "Purchase orders"}`}</span>
      </div>
